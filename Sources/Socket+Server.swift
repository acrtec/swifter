...
extension Socket {
    ...
    ///       otherwise - in IPv6.
    public class func tcpSocketForListen(_ port: in_port_t, _ forceIPv4: Bool = false, _ maxPendingConnection: Int32 = SOMAXCONN, _ listenAddress: String? = nil, _ nonblocking : Bool = false) throws -> Socket {
        ...
        if nonblocking {
            ...
            var time = timeval(tv_sec: 1200, tv_usec: 0)
            if setsockopt(socketFileDescriptor, SOL_SOCKET, SO_RCVTIMEO, &time, socklen_t(MemoryLayout<timeval>.stride)) == -1 {
                let details = Errno.description()
                Socket.close(socketFileDescriptor)
                throw SocketError.socketSettingReUseAddrFailed(details)
            }
